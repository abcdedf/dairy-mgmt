...
void main() {
  ...
  group('PouchTypeController', () {
    ...
    test('savePouchType posts with milk_per_pouch and pouches_per_crate keys', () async {
      // Stub initial fetch
      fake.onGet('/pouch-types', ApiResponse.success(statusCode: 200, data: []));
      // Stub the save POST
      fake.onPost('/pouch-types', ApiResponse.success(statusCode: 201, data: {'id': 3}));
    ...
    test('updatePouchType posts with correct keys', () async {
      ...
      fake.onPost('/pouch-types/1', ApiResponse.success(statusCode: 200, data: {'updated': true}));
      ...
      final postCalls = fake.calls.where((c) => c.method == 'POST' && c.path.contains('/pouch-types/1')).toList();
      expect(postCalls.length, 1);
    ...
    test('PouchType.fromJson parses milk_per_pouch and pouches_per_crate', () {
